...
extern void DrawCubes(uint32 &, Shader &, Texture &, Texture &, glm::vec3 &,
                      glm::vec3 &, glm::vec3 &, glm::vec3 &, float[],
                      std::vector<glm::vec3> &);
...
static void key_callback(GLFWwindow *win, int key, int scancode, int action,
	...
	switch (key) {
		...
		case GLFW_KEY_Q:
			if (action == GLFW_PRESS) z_r_change -= r_changing;
			break;
		case GLFW_KEY_E:
			if (action == GLFW_PRESS) z_r_change += r_changing;
			break;
	...
	if ((key == GLFW_KEY_UP || key == GLFW_KEY_DOWN || key == GLFW_KEY_RIGHT ||
	     key == GLFW_KEY_LEFT || key == GLFW_KEY_Q || key == GLFW_KEY_E) &&
	    action == GLFW_RELEASE) {
		x_size_change = 0;
		y_size_change = 0;
		z_size_change = 0;
...
static void mainWin() {
	...
	if (!glfwInit()) sys_error("mainWin: glfwInit error");

	GLFWwindow *win =
	    create_new_window(MW_Width, MW_Height, "Main window", nullptr, nullptr);
	...
	if (glewInit() != GLEW_OK) sys_error("mainWin: glewInit error");
...

	{
		...
		std::vector<glm::vec3> cubePos = {
			...
			glm::vec3(1.f, 1.5f, 1.5f), glm::vec3(2.f, 2.5f, 2.f),
			glm::vec3(2.5f, 1.f, 2.1f),
		};

		glm::vec3 size(1.f), camPos(0, 0, 2.f), camFront(0.f, 0.f, -1.f),
		    camUp(0.f, 1.f, 0.f);
		...
		while (!glfwWindowShouldClose(win)) {
			...
			DrawCubes(vao, program, texture1, texture2, camPos, camFront, camUp,
			          size, rotation, cubePos);
...
static void DrawCubes(uint32 &vao, Shader &program, Texture &tex1,
                      Texture &tex2, glm::vec3 &camPos, glm::vec3 &camFront,
                      glm::vec3 &camUp, glm::vec3 &size, float rotation[],
                      std::vector<glm::vec3> &cubePos) {
	tex1.Bind(0);
	tex2.Bind(1);
	...
	projection = glm::perspective(
	    glm::radians(fov), (float)MW_Width / (float)MW_Height, 0.1f, 100.0f);
...
static void action(glm::vec3 &size, float rotation[], glm::vec3 &camPos,
	...
	size[0] += x_size_change;
	size[1] += y_size_change;
	size[2] += z_size_change;
	rotation[0] += x_r_change;
	rotation[1] += y_r_change;
